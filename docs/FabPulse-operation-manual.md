...
> 以「畫面」為單位之操作說明 · 版本 2026-06-13（v1.1.0.0）· 對應分支 `master`
> 適用對象：操作人員、系統分析（SA）、教育訓練。
> 範圍：MES 7 + ERP 10 + SCM 4 共 21 個功能畫面（ATP 併於 ERP 銷售訂單畫面）＋ **Handbook 說明手冊頁**（Help 域）。
...
| 模組標題列 ModuleHeader | 每個畫面最上方 | 該畫面的戰情標題：發光標題 + 分域徽章（MES／ERP／SCM）+ 幾個即時 count-up KPI pill；可一眼判斷所在畫面與當前表現 |
| 主內容區 | 中央 | 目前所選畫面（標題列下方為該畫面專屬的即時資料主視覺與面板） |
...
| 開啟導覽中心 | 點 TopBar `?` 按鈕 或按鍵 `?` | 全域 |
| 導覽步進 | `←` / `→`（前一步 / 下一步）| 導覽進行中 |
| 暫停 / 繼續導覽 | 按鍵 `Space` | 導覽進行中 |
| 關閉導覽 | 按鍵 `Esc` 或導覽卡 ✕ | 導覽進行中 |
| 查閱操作手冊 | 側欄 Help > Handbook | 全域 |
...
### HELP-01　Handbook（操作說明手冊）
- **畫面路徑**：Help > Guidance > Handbook ／ **代碼** `handbook`
- **功能說明**：雙語（繁中 / EN）可搜尋參考手冊，記錄所有 21 個功能模組與 4 個系統主題（迴圈時鐘、事件匯流排、MES→ERP 橋接、SCM Driver），每篇皆提供「Open page（前往該頁）」與「Tour this page（啟動該頁迷你導覽）」深度連結。
- **顯示說明**：
  | 區塊 | 內容 |
  |------|------|
  | 左欄 | 搜尋框＋依域分組導覽清單（MES / ERP / SCM / SYSTEM），點條目切換右欄 |
  | 右欄 | 選取條目之說明標題、各段落（含事件、設計說明）、「Open page」與「Tour this page」按鈕 |
  | 語言切換 | 右上角 中文 / EN 切換鈕，即時切換整份手冊語言 |
- **操作步驟**：
  1. 在左欄搜尋框輸入關鍵字（模組名稱、事件名稱均可），清單即時過濾。
  2. 點左欄任一條目 → 右欄顯示該條目的完整說明。
  3. 點 **Open page** → 跳至對應功能畫面。
  4. 點 **Tour this page** → 啟動該畫面的單頁迷你導覽（涵蓋該域章節的所有步驟）。
  5. 右上語言切換鈕可在中文與英文之間切換。
- **注意事項**：Handbook 本身不含任何即時數據，為靜態說明內容；導覽啟動後須先切至該功能畫面，Handbook 畫面會自動離開。
- **關聯畫面**：Tour Center（導覽中心，由 `?` 按鈕開啟）。

### HELP-02　Tour Center（導覽中心）與導覽系統
- **入口**：TopBar 右上 `?` 按鈕 或 鍵盤 `?`；新訪客首次進入系統會彈出歡迎提示。
- **功能說明**：管理四條預設導覽（完整流程＋ MES / ERP / SCM 分域），顯示每條導覽的估計分鐘數與完成進度，可一鍵啟動或重播。
- **導覽系統特性**：
  - **自動導航**：導覽執行期間會自動切換畫面頁面，聚光燈標示當前說明的面板或元件。
  - **雙語旁白**：每步驟均有繁中 / EN 說明，可於導覽卡右上角即時切換語言。
  - **等待真實事件**：部分步驟（如批次完工、ERP 收貨）會暫停等待模擬迴圈實際觸發對應事件（最長 30 秒），以確保旁白與畫面一致。
  - **側欄點選優先**：導覽自動播放期間，若使用者點側欄切換畫面，導覽自動暫停（不衝突）；點「繼續」即接續。
  - **完成記錄**：已完成的導覽會標記，進度跨訪次保留（localStorage）。
- **鍵盤控制**：`←` / `→` 步進、`Space` 暫停 / 繼續、`Esc` 關閉。
...
*本操作功能說明書依 2026-06-13 之 `master`（v1.1.0.0：全畫面新增 ModuleHeader 戰情標題列與即時資料主視覺；含導覽系統與 Handbook）實機畫面與原始碼整理。技術架構細節另見 `docs/FabPulse-system-functional-spec.md`。*
